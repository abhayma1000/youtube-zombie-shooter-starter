...
class Agent():
    def __init__(self, env: ZombieShooter, dropout, hidden_layer, learning_rate, step_repeat, gamma):
        ...
        self.memory = ReplayBuffer(max_size=500000, input_shape=observation.shape, 
        n_actions=env.action_space.n, device=self.device)

        self.model_1 = ZombieNet(action_dim=env.action_space.n, hidden_dim=hidden_layer, dropout=dropout,
        observation_shape=observation.shape).to(self.device)
        self.model_2 = ZombieNet(action_dim=env.action_space.n, hidden_dim=hidden_layer, dropout=dropout,
        observation_shape=observation.shape).to(self.device)

        self.target_model_1 = ZombieNet(action_dim=env.action_space.n, hidden_dim=hidden_layer, dropout=dropout,
        observation_shape=observation.shape).to(self.device)
        self.target_model_2 = ZombieNet(action_dim=env.action_space.n, hidden_dim=hidden_layer, dropout=dropout,
        observation_shape=observation.shape).to(self.device)
        ...
        for episode in range(episodes):
            ...
            while not done and episode_steps < max_episode_steps:
                ...
                if self.memory.can_sample(batch_size):
                    ...
                    # Take the min of the next Q values
                    ...
                    writer.add_scalar(os.path.join("loss", "Model_1"), loss_1.item(), total_steps)
                    writer.add_scalar(os.path.join("loss", "Model_2"), loss_2.item(), total_steps)
            ...
            self.model_1.save_the_model(os.path.join("models", "dqn1.pth"))
            self.model_2.save_the_model(os.path.join("models", "dqn2.pth"))
