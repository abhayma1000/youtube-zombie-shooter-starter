...
class TreasureChest:
	def __init__(self, x, y):
		self.closed_image = pygame.image.load(os.path.join("images", "chest_closed.png")).convert_alpha()
		self.opened_image = pygame.image.load(os.path.join("images", "chest_opened.png")).convert_alpha()

		self.size = 50
		self.closed_image = pygame.transform.scale(self.closed_image, (self.size, self.size))
		self.open_image = pygame.transform.scale(self.opened_image, (self.size, self.size))
...
class HealthDrop:
	def __init__(self, x, y):
		self.image = pygame.image.load(os.path.join("images", "heart.png")).convert_alpha()
		self.size = 30
...
class ZombieShooter():
	def __init__(self, window_width, window_height, world_height, world_width, fps, sound=False, render_mode='human'):
		...
		if render_mode == "human":
			...
			raise Exception("Invalid render_mode")
		...
		if self.sound:
			...
			self.zombie_bite = pygame.mixer.Sound(os.path.join("sounds", "zombie_bite_1.wav"))
			self.zombie_hit = pygame.mixer.Sound(os.path.join("sounds", "zombie_hit.wav"))
			self.shotgun_blast = pygame.mixer.Sound(os.path.join("sounds", "shotgun_blast.wav"))
			self.zombie_snarl = pygame.mixer.Sound(os.path.join("sounds", "zombie_snarl.wav"))
			self.footstep = pygame.mixer.Sound(os.path.join("sounds", "footstep.wav"))
			self.vocals_1 = pygame.mixer.Sound(os.path.join("sounds", "one_of_those_things_got_in.wav"))
			self.vocals_2 = pygame.mixer.Sound(os.path.join("sounds", "virus_infection_alert.wav"))
			self.vocals_3 = pygame.mixer.Sound(os.path.join("sounds", "come_and_see.wav"))
	...
	def fill_background(self):
		...
		gun_type_surface = self.font.render(f"Gun type: {self.gun_type.title()}", True, (0, 0, 0))
		self.screen.blit(gun_type_surface, (10, 110))

		if self.out_of_ammo_message_displayed and self.gun_type == "shotgun":
			out_of_shotgun_ammo_surface = self.font.render(
				"Out of shotgun ammo: Press tab to switch to a single shot", True, (255, 0, 0)
			)
			out_of_ammo_rect = out_of_shotgun_ammo_surface.get_rect(center=(self.window_width // 2, self.window_height // 2))
			self.screen.blit(out_of_shotgun_ammo_surface, out_of_ammo_rect)
	...
	def _get_obs(self):
		...
		observation = torch.from_numpy(grayscale).float().unsqueeze(dim=0)
	...
	def step(self, action, repeat=4):
		...
		for i in range(repeat):
			...
			if action == 5 or action == 6: # Don't want to do the same action multiple times, just once
				action = 0
	...
	def _step(self, action):
		...
		switch_gun = False # Would be action 5
		...
		if len(self.zombies) < self.max_zombie_count and random.randint(1, 100) < 3:
			self.zombies.append(Zombie(world_height=self.world_height, 
			world_width=self.world_width, size=80, speed=random.randint(1, self.zombie_top_speed)))
		...
		if not collision and self.player.x != new_player_x and 0 <= new_player_x <= self.world_width - self.player.size:
		...
		if not collision and self.player.y != new_player_y and 0 <= new_player_y <= self.world_height - self.player.size:
		...
		if self.treasure_chest and self.player.rect.colliderect(self.treasure_chest):
			if not self.treasure_chest.is_opened:
				self.treasure_chest_is_opened = True
				reward += 1
